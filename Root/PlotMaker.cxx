...
//   }
  ...
  for(unsigned int i=0; i<m_sampleList.size(); ++i) {
    ...
    if((m_dbg || m_eventyield) && (m_sampleList.at(i)!="Data" || m_dataBlindCut>0.)) cout << m_sampleList.at(i) << " : " << histograms[i]->Integral(0, -1) << " \\\\" << endl;
...
//   if(m_eventyield) cout << "Data= " << histograms[dataIndex]->Integral(0,-1)  << endl;
//   if(m_eventyield) cout << "data content= " << histograms[dataIndex]->Integral() << endl;
  ...
  TArrow* arrow = new TArrow(m_dataBlindCut/1000.,0,m_dataBlindCut/1000.,histograms[dataIndex]->GetMaximum()/1.5,0.03,"<|");
...
void PlotMaker::buildRatioErrorBand(TGraphAsymmErrors* input, TGraphAsymmErrors* output, int w_ZVIndex) 
{
  ...
  for(int bin=0; bin < output->GetN(); bin++){ 
    ...
    if( fabs(input->GetY()[bin]) > 0.0001 ) {
      ...
      if(m_dbg || m_eventyield) cout << "$^{+ " << input->GetEYhigh()[bin] /*<< " / " << input->GetY()[bin]*/ << "} "; 
    }
   else 
   ...
   if( fabs(input->GetY()[bin]) > 0.0001 ){
      ...
      if(m_dbg || m_eventyield) cout << "_{- " << input->GetEYlow()[bin] /*<< " / " << input->GetY()[bin]*/ << "}$ \\\\" << endl;     
