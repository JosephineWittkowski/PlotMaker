...
//   totalSysHistoBg == NULL;
//   if(totalSysHistoBg==NULL) cout << "totalSysHistoBg==NULL" << endl;
...
//     cout << m_systematicsList.at(i) << endl;
	...
	if(sysHistograms[j]!=NULL){
	  cout << m_sampleList.at(j) << " " << m_systematicsList.at(i) << " Integral " << sysHistograms[j]->Integral(0,-1) << endl;
...
//     myAddtoBand(transientFakeUp,nominalAsymErrors);
//     TGraphAsymmErrors* transientFakeDn = TH1TOTGraph(fake_histo_dn);
//     myAddtoBand(transientFakeDn,nominalAsymErrors);
...
// 	  totalSysHistoBg->Add(_hsys);
	...
	TGraphAsymmErrors* nominalAsymErrorsFake = TH1TOTGraph(m_fakeHistogram);
	myAddtoBand(transientFake,nominalAsymErrorsFake); //100 uncorrelated sys - add in quad to the other MC bkg sys
	totalSysHistoBg->Reset();
      }
    }
