...
//   double err_up = 0.;
//   double err_dn = 0.;
  bool calc_fake_sys = true;
  TH1D* fake_histo_up = (TH1D*) histograms[FakesIndex]->Clone();
  TH1D* fake_histo_dn = (TH1D*) histograms[FakesIndex]->Clone();
  cout << "fakes:" << endl;
  if(m_fakesystematicsList.size()>0){
    for(int bin=1; bin<histograms[FakesIndex]->GetNbinsX()+1; ++bin){
      float stat = pow(histograms[FakesIndex]->GetBinError(bin),2);//stat err
      float err_up = stat; 
      float err_dn = stat;
      float bc = histograms[FakesIndex]->GetBinContent(bin);
      cout << "bc= " << bc << endl;
	...
	getHistograms(m_inputROOTFile, observable, cut, fakesysHistograms, m_fakesystematicsList.at(i), m_fakesampleList);

	float shift = fakesysHistograms[0]->GetBinContent(bin);
	cout << "shift= " << shift << endl;
	if(shift > bc) err_up += pow(shift-bc,2);
        else           err_dn += pow(shift-bc,2);
	cout << "err_up= " << err_up << " err_dn= " << err_dn << endl;
	fake_histo_up->SetBinContent(bin, err_up);
	fake_histo_dn->SetBinContent(bin, err_dn);
	
      }
    }
    
    fake_histo_up->Add(histograms[FakesIndex]);
    cout << "fake_histo_up->Integral()= " << fake_histo_up->Integral() << endl;
    TH1D* _htmp = (TH1D*) histograms[FakesIndex]->Clone();
