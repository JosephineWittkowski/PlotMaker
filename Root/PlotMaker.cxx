...
//     cout << " content for " << m_signalList.at(i) << " = " << histograms_signal[i]->Integral() << endl;  
  ...
  int 	   FakesIndex     = 0;
  if(m_fakesampleList.size()>0)m_fakeHistogram = (TH1D*) fakehistograms[FakesIndex]->Clone();
...
//     if(m_sampleList.at(i) == "Fakes") FakesIndex = i;
  ...
  for(unsigned int i=0; i<m_fakesampleList.size(); ++i) {
    ...
    cout << m_fakesampleList.at(i) << " NOMINAL= " << fakehistograms[i]->Integral(0, -1) << endl;
  }
//   cout << "MC stack content = " << ((TH1*)(stackBg->GetStack()->Last()))->Integral() << endl;
  ...
  for(int bin=0; bin < nominalAsymErrorsMCBg->GetN(); bin++){ 
    cout << "bin " << bin <<  " EYhigh()= " << nominalAsymErrorsMCBg->GetEYhigh()[bin] << endl;
    cout << "bin " << bin <<  " EYlow()=  " << nominalAsymErrorsMCBg->GetEYlow()[bin] << endl;
  }
  
  if(m_fakesampleList.size()>0){
    TGraphAsymmErrors*  AsymmErrorsFake = TH1TOTGraph(fakehistograms[FakesIndex]);
...
//   int McBgIndex = -1;
  for(unsigned int i=0; i < m_MCsystematicsList.size(); ++i) {
//     cout << m_MCsystematicsList.at(i) << endl;
	...
	if(sysHistograms[j]!=NULL){
	  cout << m_sampleList.at(j) << " " << m_MCsystematicsList.at(i) << " Integral " << sysHistograms[j]->Integral(0,-1) << endl;
...
//     }
//     
//     TGraphAsymmErrors* transientFakeUp = TH1TOTGraph(fake_histo_up);
//     myAddtoBand(transientFakeUp,nominalAsymErrorsMCBg);
//     TGraphAsymmErrors* transientFakeDn = TH1TOTGraph(fake_histo_dn);
//     myAddtoBand(transientFakeDn,nominalAsymErrorsMCBg);



  }
  //     string dummystring = "1,2";
//     vector <string> dummylist;
//     tokenizeString(dummystring, dummylist);
//     TH1D *FakeUpDnHistograms[dummylist.size()];
    if(m_fakesampleList.size() > 0 && m_fakesystematicsList.size()>0){
      vector<TH1D*> FakeUpDnHistograms;
      TH1D* sys0 = (TH1D*) fakehistograms[FakesIndex]->Clone();
      TH1D* sys1 = (TH1D*) fakehistograms[FakesIndex]->Clone();
      sys0->Reset();
      sys1->Reset();
      FakeUpDnHistograms.push_back(sys0);
      FakeUpDnHistograms.push_back(sys1);
      GetFakeSys(FakeUpDnHistograms, observable, cut);
      cout << "FakeUpDnHistograms[0]->Integral(0,-1)= " << FakeUpDnHistograms[0]->Integral(0,-1) << endl;
      cout << "FakeUpDnHistograms[1]->Integral(0,-1)= " << FakeUpDnHistograms[1]->Integral(0,-1) << endl;
      totalSysHistoBg->Reset();
      

    
//       totalSysHistoBg->Add(FakeUpDnHistograms[0]);
//       totalSysHistoBg->Add(FakeUpDnHistograms[1]);
      cout << "totalSysHistoBg->Integral(0,-1)= " << totalSysHistoBg->Integral(0,-1) << endl;
	...
	myAddtoBand(transientFake,nominalAsymmErrorsBg); //100 uncorrelated sys - add in quad to the other MC bkg sys
	totalSysHistoBg->Reset();
      }
    }
...
//   gPad->SetLogy(1);
  ...
  buildRatioErrorBand(nominalAsymErrorsMCBg,ratioBand);
  ...
  for(int i=1; i<=nominalAsymErrorsMCBgNoError->GetN(); ++i) {
...
// 	  cout << "variation= " << variation << " inputList.at(i)= " << inputList.at(i) << endl;
...
//     if(m_fakesystematicsList.size()>0){
      cout << "m_fakeHistogram->Integral(0,-1)= " << m_fakeHistogram->Integral(0,-1) << endl;
	...
	for(unsigned int i=0; i < m_fakesystematicsList.size(); ++i) {	
	  cout << "m_fakesystematicsList.at(i)= " << m_fakesystematicsList.at(i) << endl;
	  ...
	  cout << "err_up^2= " << err_up << " err_dn^2= " << err_dn << endl;
      ...
      cout << "fake_histo_up->Integral()= " << fake_histo_up->Integral() << endl;
      ...
      cout << "fake_histo_dn->Integral()= " << fake_histo_dn->Integral() << endl;
