...
//   }
  ...
  for(unsigned int i=0; i<m_signalList.size(); ++i) {
    ...
    if(m_eventyield) cout << " content for " << m_signalList.at(i) << " = " << histograms_signal[i]->Integral() << endl;  
  }
  ////////////////////////////////////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////////////////////////////////////
  ...
  getHistograms(m_inputROOTFile, observable, cut, fakehistograms,"CENTRAL", m_fakesampleList);
  ...
  for(unsigned int i=0; i<m_fakesampleList.size(); ++i) {
    ...
    if(m_dbg || m_eventyield) cout << m_fakesampleList.at(i) << " NOMINAL= " << fakehistograms[i]->Integral(0, -1) << endl;
  }
  if(m_eventyield) cout << "MC stack content = " << ((TH1*)(stackBg->GetStack()->Last()))->Integral() << endl;
  ...
  // 2) add for each individual sys the difference to the nominal value to yhigh or ylow with myAddtoBand. Original TGraphAsymmErrors 'nominalAsymErrorsMCBg': deduced from MC stack histo -> includes statistical error of all MC histos.
  ...
  // 5) use again myAddtoBand method. 'nominalAsymErrorsMCBg' now inlcudes all background (MC + fake !) -> add other bg contributions to fake_up and fake_dn histos before calling the method.
  ...
  for(unsigned int i=0; i < m_MCsystematicsList.size(); ++i) {
    ...
    getHistograms(m_inputROOTFile, observable, cut, sysHistograms, m_MCsystematicsList.at(i), m_sampleList);
	...
	if(sysHistograms[j]!=NULL){
	  if(m_dbg) cout << m_sampleList.at(j) << " " << m_MCsystematicsList.at(i) << " Integral " << sysHistograms[j]->Integral(0,-1) << endl;
    ...
    //2) add for each individual sys the difference to the nominal value to yhigh or ylow with myAddtoBand. Original TGraphAsymmErrors 'nominalAsymErrorsMCBg': deduced from stack histo -> includes statistical error of all MC histos.
    
    //All MC BG (correlated-> use TH1D, linear) SYS (uncorrelated -> use myAddtoBand(), squared)

      transientMcBg = TH1TOTGraph(totalSysHistoBg);
      myAddtoBand(transientMcBg,nominalAsymErrorsMCBg);
      totalSysHistoBg->Reset();
  ...
  // 3) to include stat error of fake histos, use myTGraphErrorsAdd() method to add fake_histo (= stat errors) to 'nominalAsymErrorsMCBg'.
    if(m_fakesampleList.size() > 0 && m_fakesystematicsList.size()>0){
      ...
      // 4) for fakes, don't consider stat error any more (already included!). Loop over each bin for all fake sys and add difference to nominal event yield to yhigh or ylow (in GetFakeSys() method)
      GetFakeSys(FakeUpDnHistograms, observable, cut);
      if(m_dbg){
	...
	myAddtoBand(transientFake,nominalAsymErrorsMCBg); //100 uncorrelated sys - add in quad to the other MC bkg sys
	totalSysHistoBg->Reset();
      }
    }
...
// 7) Add squared 17.4% * WZ nominal yield to total error for the uncertainty on the WZ background.
	...
	nominalAsymErrorsMCBg->GetEYhigh()[bin] = sqrt(pow(ey_high, 2) + pow( 0.174*m_ZVHistogram->GetBinContent(bin+1),2));
	if(m_dbg) cout << "yhigh set from " << ey_high << " to " << sqrt(pow(ey_high, 2) + pow( 0.174*m_ZVHistogram->GetBinContent(bin+1),2)) << endl;
	nominalAsymErrorsMCBg->GetEYlow()[bin]  = sqrt(pow(ey_low, 2) + pow( 0.174*m_ZVHistogram->GetBinContent(bin+1),2)); 
	if(m_dbg) cout << "ey_low set from " << ey_low << " to " << sqrt(pow(ey_low, 2) + pow( 0.174*m_ZVHistogram->GetBinContent(bin+1),2)) << endl;
      }
    }
  ...
  if(m_eventyield) cout << "data content= " << histograms[dataIndex]->Integral() << endl;
...
//   gPad->SetLogy(1);
  ...
  if(m_eventyield) cout << "Total = " << ((TH1*)(stackBg->GetStack()->Last()))->Integral() << endl;
...
void PlotMaker::buildRatioErrorBand(TGraphAsymmErrors* input, TGraphAsymmErrors* output, int w_ZVIndex) 
{
  ...
  for(int bin=0; bin < output->GetN(); bin++){ 
    ...
    if( input->GetY()[bin] > 0.0001 ) {
      ...
      if(m_dbg) cout << "EYhigh= " << input->GetEYhigh()[bin] << " / " << input->GetY()[bin] << endl; 
    }
   else 
   ...
   if( input->GetY()[bin] > 0.0001 ){
      ...
      if(m_dbg) cout << "EYlow=  " << input->GetEYlow()[bin] << " / " << input->GetY()[bin] << endl;     
...
//      output->GetEYhigh()[bin] = 1.; 
  ...
  void PlotMaker::GetFakeSys(vector<TH1D*> &w_fakeUpDnHistograms, TString w_observable, TString w_cut){
	...
	getHistograms(m_inputROOTFile, w_observable, w_cut, fakesysHistograms, m_fakesystematicsList.at(i), m_fakesampleList);
