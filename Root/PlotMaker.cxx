...
//     histograms_signal[i]->SetMarkerSize(1);
    histograms_signal[i]->SetLineWidth(2);
//     histograms_signal[i]->SetLineColor(SampleColors[m_signalList.at(i)]);
...
//   ratio         ->Draw("same && P");
  ...
  TString plotName = channel + "_" + region + "_" + variable + "_NOdata.pdf" ;
...
void PlotMaker::getHistograms(TFile* input, TString varToPlot, TString cutToApply, TH1D* histos[], TString variation, vector<string> inputList)
{
  ...
  for(unsigned int i=0; i<inputList.size(); ++i) {
    ...
    if( (inputList.at(i) == "Data" || inputList.at(i) == "Fakes") && !variation.EqualTo("CENTRAL") ) {
      histos[i] = NULL;
      continue;
    }
    if(inputList.at(i) != "Fakes" && (variation.Contains("ELFR")==1 || variation.Contains("ELRE")==1 || variation.Contains("MUFR")==1 || variation.Contains("MURE")==1)){
      continue;
    }
    
    //use eventweight for nominal (CENTRAL) and most sytematic variations.
    TString weight = "eventweight";
    //use other weight for systematic variations where event weight was changed.
//     if(variation == "wsysEFSUP") weight = "syst_EFSUP";
//     cout << variation << " weight= " << weight << endl;
    
    
    // Define variables internal to the loop
    TString treeName = "";
    if(inputList == m_sampleList){
      if(variation.Contains("wsys")==0)  treeName = ""; treeName.Form("%s_",inputList.at(i).c_str()); treeName += variation;
      //if variation contains wsys, use CENTRAL tree
      if(variation.Contains("wsys")==1){
	treeName = ""; treeName.Form("%s_",inputList.at(i).c_str()); treeName += "CENTRAL";
  //       treeName = "Higgs_CENTRAL";
      }
    }
    else{
      if(variation.Contains("wsys")==0)  treeName = ""; treeName.Form("SSWH8TeV_%s_",m_signalList.at(i).c_str()); treeName += variation;
    }
